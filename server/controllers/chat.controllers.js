import { ALERT, NEW_MESSAGE, NEW_MESSAGE_ALERT, REFETCH_CHATS } from "../constents/event.constents.js";
...
const getMyChats = TryCatch(async (req, res, next) => {
    ...
    const transformedChats = chats.map(({ _id, name, groupChat, members }) => {
        ...
        const otherMembers = getOtherMembers(members, users)
...
const addMembers = TryCatch(async (req, res, next) => {
    ...
    if (chat.creater.toString() !== req.user.toString()) return next(new ErrorHandler("You are not allowed to add the members", 403))
...
const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    if (!chat) return next(new ErrorHandler("Chat is not found ", 403))
    ...
    emitEvent(req, NEW_MESSAGE_ALERT, chat.members, { chat: chatId })
...
const renameGroup = TryCatch(async (req, res, next) => {
    ...
    if (!chat) return next(new ErrorHandler("Chat is not found ", 404))
    if (!chat.groupChat) return next(new ErrorHandler("This is not a group chat", 400))
    if (chat.creater.toString() !== req.user.toString()) return next(new ErrorHandler("You are not allowed to rename the group", 403))
    chat.name = name;
    await chat.save()
    emitEvent(req, REFETCH_CHATS, chat.members)
    return res.status(200).json({
        success: true,
        message: "Group renaed successfully"
    })
})
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    if (chat.groupChat && chat.creater.toString() !== req.user.toString()) return next(new ErrorHandler("You are not allowed to delete the group", 403))
    ...
    // here we have to delete all messages as well as attachments or files on cloudinary

    const messagesWithAttachments = await Message.find({ chat: chatId, attachemts: { $exists: true, $ne: [] } })
