...
const adminLogin = TryCatch(async (req, res, next) => {
    ...
    return res.status(200)
        .cookie("chatting-admin-token", token, { ...cookieOptions, maxAge: 1000 * 60 * 15 })
        .json({
            success: true,
...
const adminLogout = TryCatch(async (req, res, next) => {
    ...
    return res.status(200)
        .cookie("chatting-admin-token","", { ...cookieOptions, maxAge: 0 })
        .json({
            success: true,
...
const allChats = TryCatch(async (req, res) => {
    ...
    const transformedChats = await Promise.all(chats
        .map(async ({ members, _id, groupChat, name, creater }) => {
...
const getDashboardStats = TryCatch(async (req, res) => {
    ...
    const [groupsCount, userCounts, messagesCount, totalChatCounts] = await Promise.all([
